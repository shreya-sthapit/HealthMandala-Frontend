...
export default function SetDoctorPassword() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setDoctorName(payload.doctorName || 'Doctor');
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/doctor-auth/set-password`, {
        method: 'POST',
    ...
    return (
      <div className="ap-page">
        <div className="ap-card" style={{ maxWidth: 480, height: 'auto', padding: '3rem 2rem', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Redirecting to your doctor dashboard...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="ap-page">
        <div className="ap-card" style={{ maxWidth: 480, height: 'auto', padding: '3rem 2rem', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.875rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
            {pageStatus === 'expired'
              ? 'This link has expired (valid for 48 hours). Please contact your hospital administrator to resend the invite.'
              : 'This link is invalid or has already been used. Please contact your hospital administrator.'}
          </p>
      ...
      <div className="ap-card">
        ...
        <div className="ap-overlay-panel" style={{ left: '0', borderRadius: '20px 0 0 20px' }}>
          <div className="ap-overlay-left" style={{ opacity: 1, transform: 'translateX(0)', pointerEvents: 'all' }}>
            ...
            <p>Your doctor portal is ready. Manage your appointments, patients, and schedules — all in one place.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', width: '100%', maxWidth: '220px' }}>
              {['View appointments', 'Manage patient records', 'Update your schedule'].map(item => (
        ...
        <div className="ap-form-panel ap-signup-panel" style={{ opacity: 1, transform: 'translateX(0)', pointerEvents: 'all', justifyContent: 'center', paddingTop: '3rem', paddingBottom: '3rem' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.84rem', marginBottom: '1.2rem', width: '100%', maxWidth: 420 }}>
            Hi {doctorName}, create a secure password for your doctor account.
          </p>
